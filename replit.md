...
- **Status Transition Enforcement:** A server-side state machine (`VALID_MANUAL_STATUS_TRANSITIONS` in `claims.ts`) enforces which status transitions are allowed. System-controlled statuses (Portal Queued, Generating Email, Ready to Review) cannot be set manually — they are only set by system processes (portal submission confirmation, batch processing). The `GET /api/claims/valid-transitions/:id` endpoint returns the valid statuses, outcomes, and whether portal queuing is allowed for a given claim's current state. The frontend consumes this to dynamically render only valid options in the Actions card. Outcome changes are similarly restricted by `VALID_OUTCOME_BY_STATUS` — e.g., "Approved" can only be set on claims in Awaiting Response, Resolved, or Denied status. Portal submission retry is restricted to `failed`/`dry_run` submissions, and cancel is blocked for `in_progress`/`submitted` submissions.

...
- "info_request" — payor needs us to do something else before they can decide, OR is closing this channel and redirecting us to a DIFFERENT workflow we have not yet executed. The operator has new work to do, but no approval/denial was issued. Examples: "Please provide GPS breadcrumbs for confirmation #X", "Submit this through the GPS Deviation Control category", "Corrections - Ticket Closed … you must enter a correction through your MAS portal" ONLY when this is the FIRST time we are being told to refile via the portal (i.e. the email does NOT also say a previous correction was already submitted).
- "acknowledgment" — payor is just confirming receipt, queueing the dispute for review, explaining how their dispute process works, OR telling us this exact ticket is a duplicate of one we already filed. NO decision yet, NO new action required from us. Examples: "We have received your request", "Ticket Under Review — this ticket will be reviewed for GPS compliance" (queued for review only), "To have a leg flagged for Incomplete GPS reviewed, you must submit one ticket per invoice…" (generic process explanation, not a per-ticket directive), "Corrections can take up to 30 days before you receive a response" (status ping), "A previous correction was submitted for this on M/D/YY. Corrections - Ticket Closed" (duplicate-of-prior-submission notification — the real response is on the canonical ticket we already filed; nothing new for us to do here).
- "other" — anything else (unrelated, unclear, internal forward, etc.).

Critical distinctions:
- "Corrections - Ticket Closed" is NOT automatically an info_request. Read the surrounding lines:
    • If the email says "A previous correction was submitted for this on <date>" anywhere — it is an "acknowledgment" (this is a duplicate-ticket notification; the real reply is on the canonical per-invoice ticket).
    • If the email tells us to refile via the MAS portal and does NOT mention a prior submission — it is an "info_request" (payor closed this channel and is redirecting us to a workflow we have not yet executed).
  Never classify duplicate-ticket notifications as info_request — that would wrongly flip the canonical per-invoice group into the operator queue.

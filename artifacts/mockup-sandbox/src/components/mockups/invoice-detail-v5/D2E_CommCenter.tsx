...
/* ------------------------------------------------------------------ */
/*  D2E — "Comm-center"                                                 */
/*  Same chrome as D2. Same primitives. Same typography.                */
/*  Difference: the Communication card promotes out of the right rail   */
/*  into the center column TOP, sitting above a compressed ledger +     */
/*  focus panel. The right rail becomes Activity + Files + Notes only.  */
/*  Net effect: the conversation is the first thing you scan after KPIs.*/
/* ------------------------------------------------------------------ */

export default function D2E_CommCenter() {
  ...
  return (
    <div className="cc-scope flex flex-col h-screen overflow-hidden bg-[var(--cc-bg)] text-[var(--cc-fg)]">
      ...
      <div className="flex-1 flex overflow-hidden">
        ...
        {/* LEFT COLUMN: Summary & Actions (verbatim from D2) */}
        <div className="w-[260px] shrink-0 border-r border-[var(--cc-border)] flex flex-col overflow-y-auto bg-[var(--cc-bg)] custom-scrollbar">
          <SidebarLeft />
        </div>

        {/* CENTER COLUMN: Communication ON TOP, then ledger + focus  ←—— THE DIFFERENCE */}
        <div className="flex-1 border-r border-[var(--cc-border)] flex flex-col bg-[var(--cc-bg)] overflow-y-auto custom-scrollbar">

          {/* Communication thread — promoted to center top, full center width */}
          <div className="p-4 pb-0">
            <CommunicationCard />
          </div>

          {/* Ledger section header — verbatim from D2 */}
          <div className="flex-none mt-4 px-4 py-3 border-t border-b border-[var(--cc-border)] flex items-center justify-between bg-[var(--cc-card)] shadow-sm">
          ...
          {/* Ledger list — verbatim from D2 (LegRow + same wrapper classes) */}
          <div className="flex-none p-4 space-y-2 border-b border-[var(--cc-border)] bg-[var(--cc-muted)]/30 shadow-inner">
          ...
          {/* Focus panel (uses D2's FocusPanel verbatim) */}
          <FocusPanel leg={selected} />
        </div>

        {/* RIGHT COLUMN: Activity + Files + Notes (no comm here — it's in the center) */}
...
/* ------------------------------------------------------------------ */
/*  Communication card — verbatim markup from D2's right rail          */
/* ------------------------------------------------------------------ */
...
/* ------------------------------------------------------------------ */
/*  Right sidebar — comm REMOVED (it lives in the center). Activity +   */
/*  Group Files + Notes remain, verbatim markup from D2.                */
/* ------------------------------------------------------------------ */

function SidebarRightNoComm() {
  return (
    <div className="p-4 space-y-4">
      <div className="cc-card overflow-hidden">
        <div className="bg-[var(--cc-muted)]/50 px-3 py-2 border-b border-[var(--cc-border)]">

...
| `claims.attestation_state` (text) | **REVISED 2026-05-06: KEEP.** Initial recommendation to drop was wrong — see `docs/architecture/state-migration-plan.md` §A7. The column has live consumers in `routes/claims.ts`, `routes/invoice-groups.ts`, `routes/dashboard.ts`, `lib/attestation.ts`, components, and ~50 OpenAPI references. The per-group `reattestCompletedAt` is the **invoice-level** terminal stamp; per-leg `attestation_state` is the **leg-level** queue column. Not duplicates. | Optional v3: promote to pgEnum. No data migration. |

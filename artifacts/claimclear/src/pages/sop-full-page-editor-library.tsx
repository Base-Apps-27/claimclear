...
import { Search, FileText, ListTree, Loader2, Inbox } from "lucide-react";
        ...
        <Tabs defaultValue="evidence" className="flex-1 flex flex-col min-h-0">
          ...
          <TabsContent value="evidence" className="flex-1 overflow-y-auto px-4 py-2 mt-0">
            {isLoading ? (
              ...
              <EmptyState
                icon={evidenceItems.length === 0 ? Inbox : Search}
                title={evidenceItems.length === 0 ? "No evidence saved yet" : "No matches"}
                description={
                  evidenceItems.length === 0
                    ? 'Use "Save evidence to library" on any evidence row to add reusable items here.'
          ...
          <TabsContent value="sub_trees" className="flex-1 overflow-y-auto px-4 py-2 mt-0">
            {isLoading ? (
              ...
              <EmptyState
                icon={subTreeItems.length === 0 ? Inbox : Search}
                title={subTreeItems.length === 0 ? "No sub-trees saved yet" : "No matches"}
                description={
                  subTreeItems.length === 0
                    ? 'Use "Save sub-tree to library" on any question node to add reusable branches here.'
                    : "Try a different search term."
                }
              />
            ) : (

...
export const ListInvoiceGroupsResponse = zod.object({
  ...
  "status": zod.enum(['New', 'Needs Review', 'Needs Evidence', 'Processed', 'Portal Queued', 'Generating Email', 'Ready to Review', 'Awaiting Response', 'On Hold', 'Resolved', 'Denied']),
  "outcome": zod.enum(['Pending', 'Approved', 'Denied', 'Partially Approved', 'Non-Issue', 'Withdrawn']),
  ...
  "macroPhase": zod.union([zod.literal('pre-submit'),zod.literal('in-flight'),zod.literal('response-pending'),zod.literal('mas-action-required'),zod.literal('awaiting-payout'),zod.literal('closed'),zod.literal('on-hold'),zod.literal(null)]).nullish().describe('Server-derived macro phase used by the per-invoice transition surfaces. Only populated by endpoints that depend on it (group detail, MAS list, etc.).'),
...
export const GetDashboardSummaryResponse = zod.object({
  ...
  "reclaimedApproved": zod.string().optional().describe('Σ approvedAmount across the portfolio, RAW (no prepay multiplier — once approved, the payor remit washes the prepay through). Approved dollars on rows whose re-attestation deadline slipped are EXCLUDED — they roll into lostExpired above as a full claim loss.')
}),
...
export const GetMyProcessedTodayResponse = zod.object({
  "count": zod.number().min(getMyProcessedTodayResponseCountMin).describe('Number of claims the current user transitioned into the\n`Processed` status since the start of \"today\" in the supplied\ntimezone.\n'),
...
}).describe('Personal \"claims processed today\" counter for the streak pip on the\nsidebar avatar. The pip is private — only the requesting user\'s count\nis returned.\n')

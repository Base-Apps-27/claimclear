...
-- Performance: every read path now casts `claims.date` through
-- `NULLIF(date,'')::date` for calendar-correct sort/filter. The existing
-- text-shaped `claims_date_idx` can't service that expression, so add a
-- partial expression index over the parsed date. Partial (date <> '') so
-- it stays small and matches the cast our queries actually issue.
-- CONCURRENTLY can't run inside a transaction; intentionally separated
-- from the UPDATE block above.
CREATE INDEX IF NOT EXISTS claims_date_parsed_idx
  ON claims ((NULLIF(date, '')::date))
